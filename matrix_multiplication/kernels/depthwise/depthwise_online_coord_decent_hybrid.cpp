...
std::vector<std::vector<std::vector<std::vector<float>>>> input(BATCH_SIZE,
    std::vector<std::vector<std::vector<float>>>(CHANNELS,
    std::vector<std::vector<float>>(HEIGHT + 2 * PADDING, std::vector<float>(WIDTH + 2 * PADDING, 1.0f)))); // Padded input

std::vector<std::vector<std::vector<std::vector<float>>>> filters(CHANNELS,
    std::vector<std::vector<std::vector<float>>>(1,
    std::vector<std::vector<float>>(FILTER_SIZE,
    std::vector<float>(FILTER_SIZE, 1.0f)))); 

int output_height = (HEIGHT + 2 * PADDING - DILATION * (FILTER_SIZE - 1) - 1) / STRIDE + 1;
int output_width = (WIDTH + 2 * PADDING - DILATION * (FILTER_SIZE - 1) - 1) / STRIDE + 1;

std::vector<std::vector<std::vector<std::vector<float>>>> output(BATCH_SIZE,
    std::vector<std::vector<std::vector<float>>>(CHANNELS,
...
int measure_performance(int ts1, int ts2, int ts3, int ts4, int runs) {
  ...
  for (int i = 0; i < runs; ++i) {
    ...
    depthwise_conv2d_tiled(input, filters, output, BATCH_SIZE, CHANNELS, output_height, output_width,
                           FILTER_SIZE, STRIDE, PADDING, DILATION, ts1, ts2, ts3, ts4);
...
std::vector<std::tuple<int, int, int, int>> find_neighbors(int ts1, int ts2, int ts3, int ts4) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (it != allowed_values.end()) {
      if (it != allowed_values.begin()) {
        int prev1 = *(it - 1);
        neighbors.emplace_back((i == 0) ? prev1 : ts1, (i == 1) ? prev1 : ts2, (i == 2) ? prev1 : ts3, (i == 3) ? prev1 : ts4);
        if (it - 1 != allowed_values.begin()) {
          int prev2 = *(it - 2);
          neighbors.emplace_back((i == 0) ? prev2 : ts1, (i == 1) ? prev2 : ts2, (i == 2) ? prev2 : ts3, (i == 3) ? prev2 : ts4);
        }
      }
      if (it + 1 != allowed_values.end()) {
        int next1 = *(it + 1);
        neighbors.emplace_back((i == 0) ? next1 : ts1, (i == 1) ? next1 : ts2, (i == 2) ? next1 : ts3, (i == 3) ? next1 : ts4);
        if (it + 2 != allowed_values.end()) {
          int next2 = *(it + 2);
          neighbors.emplace_back((i == 0) ? next2 : ts1, (i == 1) ? next2 : ts2, (i == 2) ? next2 : ts3, (i == 3) ? next2 : ts4);
        }
      }
    }
  }
...
std::tuple<int, int, int, int, int> move_to_minimum(int ts1, int ts2, int ts3, int ts4, int runs) {
  auto neighbors = find_neighbors(ts1, ts2, ts3, ts4);
  int best_performance = measure_performance(ts1, ts2, ts3, ts4, runs);
  std::tuple<int, int, int, int> best_config = std::make_tuple(ts1, ts2, ts3, ts4);

  for (auto &neighbor : neighbors) {
    int n1 = std::get<0>(neighbor), n2 = std::get<1>(neighbor), n3 = std::get<2>(neighbor), n4 = std::get<3>(neighbor);
  ...
  return std::make_tuple(std::get<0>(best_config), std::get<1>(best_config), std::get<2>(best_config), std::get<3>(best_config), best_performance);
}

std::tuple<int, int, int, int, int> coordinate_descent(int ts1, int ts2, int ts3, int ts4, int runs) {
  std::cout << "Starting position: (" << ts1 << ", " << ts2 << ", " << ts3 << ", " << ts4
            << ") with performance: " << measure_performance(ts1, ts2, ts3, ts4, runs)
            << " ms\n";
  ...
  while (true) {
    auto next_position = move_to_minimum(std::get<0>(current_position),
                                         ...
                                         std::get<3>(current_position), runs);
    ...
    std::cout << "tile size (t1, t2, t3, t4): (" << std::get<0>(current_position) << ", "
              << std::get<1>(current_position) << ", " << std::get<2>(current_position)
              << ", " << std::get<3>(current_position) << ") with performance: " << current_performance << " ms\n";
  }
}

std::tuple<int, int, int, int, int> exhaustive_search(int runs) {
  int best_ts1 = 0, best_ts2 = 0, best_ts3 = 0, best_ts4 = 0, best_performance = INT_MAX;
    ...
    for (int ts2 : allowed_values) {
      for (int ts3 : allowed_values) {
        for (int ts4 : allowed_values) {
          ...
          std::cout << ts1 << " " << ts2 << " " << ts3 << " " << ts4 << " " << performance << "\n";
  ...
  return std::make_tuple(best_ts1, best_ts2, best_ts3, best_ts4, best_performance);
}
      ...
      for (int ts3 : allowed_values) {
        for (int ts4 : allowed_values) {
          ...
          if (performance < best_performance) {
            ...
            best_ts3 = ts3;
            best_ts4 = ts4;
            std::cout << "tile size (t1, t2, t3, t4): " << best_ts1 << " " << best_ts2 << " " << best_ts3 << " " << best_ts4 << " " << best_performance << "ms \n";
          }
        }
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "\ncombined_exhaustive_search:\n";
  auto result = combined_exhaustive_search(runs);
  std::cout << "Converged to: (" << std::get<0>(result) << ", " << std::get<1>(result) << ", " << std::get<2>(result) << ", " << std::get<3>(result) << ") with performance: " << std::get<4>(result) << " ms\n";
