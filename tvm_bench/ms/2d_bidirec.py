...
def bidirec(A: te.Tensor) -> te.Tensor:
    A = te.compute((N, N), lambda i, j: te.sum(A[i, j] * A[i, N-1-j], axis=0), name="B")
    return A
...
class Main:
    @T.prim_func
    def main(A: T.Buffer((10068, 10068), "float32"), B: T.Buffer((10068, 10068), "float32")):
        T.func_attr({"tir.noalias": T.bool(True)})
        # with T.block("root"):
        for i, j in T.grid(10068, 10068):
            with T.block("B"):
                v_i, v_j = T.axis.remap("SS", [i, j])
                T.reads(A[v_i, T.min(v_j, 10067 - v_j):T.min(v_j, 10067 - v_j) + (T.max(v_j, 10067 - v_j) + 1 - T.min(v_j, 10067 - v_j))])
                T.writes(B[v_i, v_j])
                B[v_i, v_j] = A[v_i, v_j] * A[v_i, 10067 - v_j]
