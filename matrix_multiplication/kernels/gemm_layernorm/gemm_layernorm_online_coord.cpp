...
vector<vector<float>> C(input_height, vector<float>(input_width, 0));

void gemm_layernorm_tiled(const vector<vector<float>>& A, const vector<vector<float>>& B, int tile_size_m, int tile_size_n, int tile_size_k);
...
int measure_performance(int ts1, int ts2, int ts3, int runs) {
  ...
  for (int i = 0; i < runs; ++i) {
    ...
    gemm_layernorm_tiled(A, B, ts1, ts2, ts3);
...
void gemm_layernorm_tiled(const vector<vector<float>>& A, const vector<vector<float>>& B, int tile_size1, int tile_size2, int tile_size3) {
    int lbv, ubv;
    float mean = 0.0;
    float variance = 0.0;
