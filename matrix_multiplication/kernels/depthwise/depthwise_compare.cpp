...
void depthwise_conv2d_tiled(
    ...
    const vector<vector<vector<vector<float>>>> &filters,
    vector<vector<vector<vector<float>>>> &output, int batch_size,
    int input_channels, int output_height, int output_width, int filter_size,
    int stride, int padding, int dilation, int tile_size_batch,
...
bool compare_outputs(const vector<vector<vector<vector<float>>>> &output1,
                     const vector<vector<vector<vector<float>>>> &output2) {
  if (output1.size() != output2.size())
    return false;
  for (size_t n = 0; n < output1.size(); ++n) {
    if (output1[n].size() != output2[n].size())
      return false;
    for (size_t c = 0; c < output1[n].size(); ++c) {
      if (output1[n][c].size() != output2[n][c].size())
        return false;
      for (size_t i = 0; i < output1[n][c].size(); ++i) {
        if (output1[n][c][i].size() != output2[n][c][i].size())
          return false;
...
int main() {
  ...
  vector<vector<vector<vector<float>>>> output_tiled(
      batch_size, vector<vector<vector<float>>>(
                      input_channels,
                      vector<vector<float>>(
                          output_height, vector<float>(output_width, 0.0f))));
  ...
  for (auto [tile_size_batch, tile_size_channel, tile_size_height,
             tile_size_width] : tile_sizes) {
    cout << "Testing with tile sizes: (batch, channel, height, width) = ("
         << tile_size_batch << ", " << tile_size_channel << ", "
         << tile_size_height << ", " << tile_size_width << ")" << endl;

    for (int n = 0; n < batch_size; ++n) {
      for (int c = 0; c < input_channels; ++c) {
        for (int i = 0; i < output_height; ++i) {
          fill(output_tiled[n][c][i].begin(), output_tiled[n][c][i].end(),
               0.0f);
        }
      }
    }
    ...
    depthwise_conv2d_tiled(
        input, filters, output_tiled, batch_size, input_channels, output_height,
    ...
    if (compare_outputs(output_non_tiled, output_tiled)) {
      cout
          << "Outputs match for tile sizes: (batch, channel, height, width) = ("
          << tile_size_batch << ", " << tile_size_channel << ", "
          << tile_size_height << ", " << tile_size_width << ")" << endl;
    } else {
      cout << "Outputs do NOT match for tile sizes: (batch, channel, height, "
              "width) = ("
           << tile_size_batch << ", " << tile_size_channel << ", "
           << tile_size_height << ", " << tile_size_width << ")" << endl;
    }
