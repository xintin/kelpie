...
#define MATRIX_SIZE 20000 //16384  // Fixed matrix size

__global__ void matMulUnrolled(float* B, float* C, float* A, int Width) {
    ...
    if (tx < Width && ty < Width) {
        ...
        for (int k = 0; k < Width; k += 8) {  // Unrolled loop with step size of 8
            Pvalue += B[ty * Width + (k + 0)] * C[(k + 0) * Width + tx];
            Pvalue += B[ty * Width + (k + 1)] * C[(k + 1) * Width + tx];
            Pvalue += B[ty * Width + (k + 2)] * C[(k + 2) * Width + tx];
            Pvalue += B[ty * Width + (k + 3)] * C[(k + 3) * Width + tx];
            Pvalue += B[ty * Width + (k + 4)] * C[(k + 4) * Width + tx];
            Pvalue += B[ty * Width + (k + 5)] * C[(k + 5) * Width + tx];
            Pvalue += B[ty * Width + (k + 6)] * C[(k + 6) * Width + tx];
            Pvalue += B[ty * Width + (k + 7)] * C[(k + 7) * Width + tx];
        }
...
float runKernel(float* d_A, float* d_B, float* d_C, int matrixSize, dim3 blockDim) {
    ...
    dim3 gridDim(108, 4192);
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);

    cudaEventRecord(start);
    matMulUnrolled<<<gridDim, blockDim>>>(d_B, d_C, d_A, matrixSize);  // Call matMul6
...
int main() {
    ...
    std::vector<int> blockSizes = {8, 16, 32, 64, 128, 256, 1024, 2048};
